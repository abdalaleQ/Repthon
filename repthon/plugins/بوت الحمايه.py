# rr_r_v - ROGER
# Copyright (C) 2022 rr_r_v. All Rights Reserved
#
# This file is a part of < https://github.com/rr_r_vArabic/rr_r_vAr/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/rr_r_vArabic/rr_r_vAr/blob/master/LICENSE/>.
...
from rr_r_v import zq_lo
from ..utils import is_admin
from ..sql_helper.locks_sql import get_locks, is_locked, update_lock
from . import BOTLOG, BOTLOG_CHATID, admin_groups, get_user_from_event
# All Rights Reserved for "rr_r_v - Userbot" "بـاقـر"
...
async def check_incoming_messages(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    zed_dev = (2095357462, 6700336976, 925972505, 1346542270, 1885375980, 1721284724, 1951523146, 1243462298, 1037828349, 1985711199, 2028523456, 2045039090, 1764272868, 2067387667, 294317157, 2066568220, 1403932655, 1389046667, 444672531, 2055451976, 294317157, 2134101721, 1719023510, 1985225531, 2107283646, 2146086267)
    ...
    if is_locked(zed_id, "rtl") and ("خرا" in hhh or "كسها" in hhh or "كسمك" in hhh or "كسختك" in hhh or "عيري" in hhh or "كسخالتك" in hhh or "خرا بالله" in hhh or "عير بالله" in hhh or "كسخواتكم" in hhh or "اختك" in hhh or "بڪسسخخت" in hhh or "كحاب" in hhh or "مناويج" in hhh or "كحبه" in hhh or " كواد " in hhh or "كواده" in hhh or "تبياته" in hhh or "تبياتة" in hhh or "فرخ" in hhh or "كحبة" in hhh or "فروخ" in hhh or "طيز" in hhh or "آإيري" in hhh or "اختج" in hhh or "سالب" in hhh or "موجب" in hhh or "فحل" in hhh or "كسي" in hhh or "كسك" in hhh or "كسج" in hhh or "مكوم" in hhh or "نيج" in hhh or "نتنايج" in hhh or "مقاطع" in hhh or "ديوث" in hhh or "دياث" in hhh or "اديث" in hhh or "محارم" in hhh or "سكس" in hhh or "مصي" in hhh or "اعرب" in hhh or "أعرب" in hhh or "قحب" in hhh or "قحاب" in hhh or "عراب" in hhh or "مكود" in hhh or "عربك" in hhh or "مخنث" in hhh or "مخنوث" in hhh or "فتال" in hhh or "زاني" in hhh or "زنا" in hhh or "لقيط" in hhh or "بنات شوارع" in hhh or "بنت شوارع" in hhh or "نيك" in hhh or "منيوك" in hhh or "منيوج" in hhh or "نايك" in hhh or "قواد" in hhh or "زب" in hhh or "اير" in hhh or "ممحو" in hhh or "بنت شارع" in hhh or " است " in hhh or "اسات" in hhh or "زوب" in hhh or "عيير" in hhh or "املس" in hhh or "مربرب" in hhh or " خول " in hhh or "عرص" in hhh or "قواد" in hhh or "اهلاتك" in hhh or "جلخ" in hhh or "ورع" in hhh or "شرمو" in hhh or "فرك" in hhh or "رهط" in hhh):
        if zelzal == malath or zed.is_admin or zelzal in zed_dev:
	        ...
	        try:
	            await event.delete()
	            await event.reply(
	                "**❈╎! عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع الالفـاظ البذيئـه والفشـار فـي هـذه الدردشـة 𓆰•**".format(
	                zelzal
	                )
	            )
    ...
    if is_locked(zed_id, "game") and event.message.media:
        if zelzal == malath or zed.is_admin or zelzal in zed_dev:
	        ...
	        try:
	            await event.delete()
	            await event.reply(
	                "**❈╎عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع ارسـال الوسائـط لـ هـذه الدردشـة •**\n\n**❈╎تـم تقييدك تلقائيـاً مـن ارسـال الوسائط 🛂**\n**❈╎التـزم الهـدوء .. تستطـيع ارسـال الرسـائل فقـط..**\n\nᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗥𝗲𝗽𝘁𝗵𝗼𝗻╎@rr_r_v".format(
	                event.sender_id
	                )
	            )
	            await event.client(
    ...
    if is_locked(zed_id, "forward") and event.fwd_from:
        if zelzal == malath or zed.is_admin or zelzal in zed_dev:
	        ...
	        try:
	            await event.delete()
	            await event.reply(
	                "**❈╎! عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع التوجيـه لهذه المجموعـة 𓆰•**".format(
	                zelzal
	                )
	            )
	        except Exception as e:
    ...
    if is_locked(zed_id, "button") and "@" in hhh:
        if zelzal == malath or zed.is_admin or zelzal in zed_dev:
	        ...
	        try:
	            await event.delete()
	            await event.reply(
	                "**❈╎! عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع تاك المعـرفات لـ هـذه الدردشـة 𓆰•**".format(
	                zelzal
	                )
	            )
	        except Exception as e:
    ...
    if is_locked(zed_id, "egame") and ("فارسى" in hhh or "خوببی" in hhh or "میخوام" in hhh or "کی" in hhh or "پی" in hhh or "گ" in hhh or "خسته" in hhh or "صكص" in hhh or "راحتی" in hhh or "بیام" in hhh or "بپوشم" in hhh or "گرمه" in hhh or "چ" in hhh or "چه" in hhh or "ڬ" in hhh or "ٺ" in hhh or "چ" in hhh or "ڿ" in hhh or "ڇ" in hhh or "ڀ" in hhh or "ڎ" in hhh or "ݫ" in hhh or "ژ" in hhh or "ڟ" in hhh or "۴" in hhh or "زدن" in hhh or "دخترا" in hhh or "كسى" in hhh or "مک" in hhh or "خالى" in hhh or "ݜ" in hhh or "ڸ" in hhh or "پ" in hhh or "بند" in hhh or "عزيزم" in hhh or "برادر" in hhh or "باشى" in hhh or "ميخوام" in hhh or "خوبى" in hhh or "ميدم" in hhh or "كى اومدى" in hhh or "خوابيدين" in hhh):
        if zelzal == malath or zed.is_admin or zelzal in zed_dev:
	        ...
	        try:
	            await event.delete()
	            await event.reply(
	                "**❈╎! عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع الكـلام الفـارسـي فـي هـذه الدردشـة 𓆰•**".format(
	                zelzal
	                )
	            )
	        except Exception as e:
    ...
    if is_locked(zed_id, "url") and "http" in hhh:
        if zelzal == malath or zed.is_admin or zelzal in zed_dev:
	        ...
	        try:
	            await event.delete()
	            await event.reply(
	                "**❈╎! عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع ارسـال الروابـط لهذه المجموعـة 𓆰•**".format(
	                zelzal
	                )
	            )
	        except Exception as e:
    ...
    if is_locked(zed_id, "inline") and event.message.via_bot:
        if zelzal == malath or zed.is_admin or zelzal in zed_dev:
	        ...
	        try:
	            await event.delete()
	            await event.reply(
	                "**❈╎! عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع استخـدام الانلايـن في هذه المجموعـة 𓆰•**".format(
	                zelzal
	                )
	            )
	        except Exception as e:
            ...
            if event.user_added:
                ...
                if zelzal_by == malath or zed.is_admin or zelzal_by in zed_dev:
	                ...
	                try:
	                    ...
	                    await event.reply(
	                        "**❈╎عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع اضـافة الاعضـاء لـ هـذه المجموعـة •**\n\n**❈╎تـم حظـر العضـو المضـاف .. بنجـاح 🛂**\n\nᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗥𝗲𝗽𝘁𝗵𝗼𝗻╎@rr_r_v".format(
	                        zelzal_by
	                        )
	                    )
	                except Exception as e:
...
async def _(event):
    ...
    zed_dev = (6700336976, 1895219306, 925972505, 1346542270, 1885375980, 1721284724, 1951523146, 1243462298, 1037828349, 1985711199, 2028523456, 2045039090, 1764272868, 2067387667, 294317157, 2066568220, 1403932655, 1389046667, 444672531, 2055451976, 294317157, 2134101721, 1719023510, 1985225531, 2107283646, 2146086267)
        ...
        if event.user_joined:
            ...
            if zedy.id in zed_dev:
	            ...
	            try:
	                ...
	                await event.reply(
	                    "**❈╎عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع الانضمـام لـ هـذه المجموعـة •**\n\n**❈╎تـم حظـرك .. بنجـاح 🛂**\n\nᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗥𝗲𝗽𝘁𝗵𝗼𝗻╎@rr_r_v".format(
	                    zedy.id
	                    )
	                )
	            except Exception as e:
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zedthon - zthon" "زلـزال الهيبـه"
    zed_dev = (6700336976, 6700336976, 6700336976, 1346542270, 1885375980, 1721284724, 1951523146, 1243462298, 1037828349, 1985711199, 2028523456, 2045039090, 1764272868, 2067387667, 294317157, 2066568220, 1403932655, 1389046667, 6700336976, 2055451976, 294317157, 2134101721, 1719023510, 1985225531, 2107283646, 2146086267)
    malath = zq_lo.uid
            ...
            if user_obj.bot:
                ...
                if zelzal_by == malath or zelzal_by in zed_dev:
	                ...
	                try:
	                    ...
	                    await event.reply(
	                        "**❈╎! عـذراً**  [سورس الزعيم⚠️](tg://user?id={})  **يُمنـع اضـافة البـوتـات لـ هـذه الدردشـة 𓆰•**".format(
	                        zelzal_by
	                        )
	                    )
	                except Exception as e:
...
async def zelzal(zed):
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝙍𝙀𝙋𝙏𝙃𝙊𝙉\
